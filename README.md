...
Please ensure you have created your application in our YOTI dashboard and have downloaded/made a note of all the keys. Keep them safe as you'll need them soon!

## Supported Versions
The Yoti Nodejs SDK currently supports all versions for Mac users. For Window users please ensure you have version 7.8.0 or later.
...
```
See node/example/simple-login/package.json for example version support.
...
See node/example/simple-login/index.js for example version support.
...
```
See ROUTES FOR OUR WEB APPLICATION section in node/example/simple-login/index.js for example version support. 
...
Update your Call back URL in our YOTI dashboard and you should be good to go!
## Running tests
If would like to run the example please test using the following command:
```npm test```
