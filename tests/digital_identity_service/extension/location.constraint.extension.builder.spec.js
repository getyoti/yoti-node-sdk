const {
  DigitalIdentityBuilders: { LocationConstraintExtensionBuilder },
} = require('../../..');
const Extension = require('../../../src/digital_identity_service/extension/extension');
...
describe('LocationConstraintExtensionBuilder', () => {
  it('should fail for latitude too low', () => {
    const builder = new LocationConstraintExtensionBuilder().withLatitude(-91);
    expect(() => builder.build()).toThrow(
      new RangeError("'latitude' value '-91' is less than '-90'")
    );
  });
  ...
  it('should fail for latitude too high', () => {
    const builder = new LocationConstraintExtensionBuilder().withLatitude(91);
    expect(() => builder.build()).toThrow(
      new RangeError("'latitude' value '91' is greater than '90'")
    );
  });
  ...
  it('should fail for longitude too low', () => {
    const builder = new LocationConstraintExtensionBuilder()
      ...
      .withLongitude(-181);
    expect(() => builder.build()).toThrow(
      new RangeError("'longitude' value '-181' is less than '-180'")
    );
  });
  ...
  it('should fail for longitude too high', () => {
    const builder = new LocationConstraintExtensionBuilder()
      ...
      .withLongitude(181);
    expect(() => builder.build()).toThrow(
      new RangeError("'longitude' value '181' is greater than '180'")
    );
  });
  ...
  it('should fail for radius less than zero', () => {
    const builder = new LocationConstraintExtensionBuilder()
      ...
      .withRadius(-1);
    expect(() => builder.build()).toThrow(
      new RangeError("'radius' value '-1' is less than '0'")
    );
  });
  ...
  it('should fail for uncertainty less than zero', () => {
    ...
    expect(() => builder.build()).toThrow(
      new RangeError("'maxUncertainty' value '-1' is less than '0'")
    );
  });
