...
const { AmlService, performAmlCheck } = require('../../src/aml_service');
  ...
  describe('#performAmlCheck', () => {
    ...
    describe('with a valid profile', () => {
      ...
      beforeEach((done) => {
        nock(`${config.yoti.connectApi}`)
          .post(/^\/api\/v1\/aml-check?/, amlPayload.getPayloadData())
          .reply(200, amlCheckResult);
        done();
      });

      it('should return a successful result from performAmlCheck call', (done) => {
        performAmlCheck(amlProfile, privateKeyFile, 'stub-app-id')
          .then((amlResult) => {
    ...
    describe('with an empty profile', () => {
      it('should throw validation error', () => {
        expect(() => performAmlCheck('', privateKeyFile, 'stub-app-id'))
          .toThrow(new Error('Error - AmlProfile should be an object of Type/AmlProfile'));
      });
    });
  });

  describe('AmlService class', () => {
    ...
    beforeAll(() => {
      amlService = new AmlService('stub-app-id', privateKeyFile, { apiUrl });
    });

    const setupResponse = (responseBody, responseStatusCode = 200) => {
      nock(apiUrlDomain)
        .post(new RegExp(`${apiUrlPath}/aml-check`))
    ...
    describe('#performAmlCheck', () => {
      ...
      describe('with an invalid profile response', () => {
        ...
        beforeEach(() => {
          const amlCheckError = fs.readFileSync('./tests/sample-data/responses/aml-check-error.json', 'utf8');
          setupResponse(amlCheckError, 400);
        });
